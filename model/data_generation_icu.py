...
class Generator():
    def __init__(self,cohort_output,if_mort,if_admn,if_los,feat_cond,feat_proc,feat_out,feat_chart,feat_med,feat_anti,feat_vent,impute,include_time,bucket):#predW,,is_data_all=False
        #self.feat_cond,self.feat_proc,self.feat_out,self.feat_chart,self.feat_med = feat_cond, feat_proc, feat_out,feat_chart,feat_med,feat_anti,feat_vent
    ...
    def generate_adm(self):
        ...
        data['charttime']=np.where(data['charttime']=='0',data['intime'],data['charttime'])#pd.Timedelta(1, unit='D')
    ...
    def generate_anti(self):
        ...
        del anti['sanity']
        #anti=pd.merge(anti,self.data[['stay_id','los']],on='stay_id',how='left')
    ...
    def generate_vent(self):
        ...
        del vent['sanity']
        #vent=pd.merge(vent,self.data[['stay_id','los']],on='stay_id',how='left')
    ...
    def mortality_length(self,include_time):#,predW
        ...
        #self.data=self.data[(self.data['los']>=include_time+predW)]#大于included time和predictWindows的住院时长
    ...
    def smooth_meds(self,bucket):
        ...
        for i in tqdm(range(0,self.los,bucket)): 
            ...
            ###ANTI
            if(self.feat_anti):
                #sub_anti=self.anti[(self.anti['start_time']>=i) & (self.anti['start_time']<i+bucket)].groupby(['stay_id','antibiotic']).agg({'subject_id':'max'})
            ...
            ###VENT
            if(self.feat_vent):
                #sub_vent=self.vent[(self.vent['start_time']>=i) & (self.vent['start_time']<i+bucket)].groupby(['stay_id','ventilation_status']).agg({'subject_id':'max'})
...
#         else:
    ...
    def smooth_meds_all(self,bucket):
        final_meds=pd.DataFrame()
        final_meds1=pd.DataFrame()
        final_proc=pd.DataFrame()
        final_proc1=pd.DataFrame()
        final_out=pd.DataFrame()
        final_out1=pd.DataFrame()
        final_chart=pd.DataFrame()
        final_chart1=pd.DataFrame()
        final_anti=pd.DataFrame()
        final_anti1=pd.DataFrame()
        final_vent=pd.DataFrame()
        final_vent1=pd.DataFrame()
        final_data=pd.DataFrame()
        ...
        for hid in tqdm(self.hids):
            #for i in tqdm(range(48,self.los,bucket)):
            d1=self.data[self.data['stay_id']==hid]
            h=0
            for i in tqdm(range(48,len(d1),bucket)):
                ###DATA
                sub_data=self.data[self.data['mdr_los']!=0]
                sub_data=sub_data[sub_data['mdr_los']<=i+bucket].groupby(['stay_id']).agg({'subject_id':'max','icu_mdr':'max','non_icu_mdr':'max','charttime':'max','intime':'max','outtime':'max','mdr_los':'max'})
                sub_data=sub_data.reset_index()
                #sub_data['mdr_los']=h+8
                sub_data['seq']=int((i+bucket)/bucket -1)
                if final_data1.empty:
                    final_data1=sub_data
                else:
                    final_data1=pd.concat([final_data1,sub_data])
                h=h+1
            if final_data.empty:
            ...
            ###MEDS
            if(self.feat_med):
                m1=self.meds[self.meds['stay_id']==hid]
                t=0
                for i in tqdm(range(0,len(m1),bucket)): 
                    #sub_meds=self.meds[(self.meds['start_time']>=i) & (self.meds['start_time']<i+bucket)].groupby(['stay_id','itemid','orderid']).agg({'stop_time':'max','subject_id':'max','rate':np.nanmean,'amount':np.nanmean})
                    sub_meds=self.meds[(self.meds['start_time']>=i) & (meds['start_time']<i+bucket)].groupby(['stay_id','itemid','orderid']).agg({'start_time':'max','stop_time':'max','subject_id':'max','mdr_los':'min','rate':np.nanmean,'amount':np.nanmean})
                    sub_meds=sub_meds.reset_index()
                    sub_meds['mdr_label']=np.where((sub_meds['mdr_los']>i)&(sub_meds['mdr_los']<=i+bucket),1,0)
                    ...
                    sub_meds['stop_time']=sub_meds['stop_time']/bucket
                    sub_meds['seq']=int((i+bucket)/bucket -1)
                    if final_meds1.empty:
                        final_meds1=sub_meds
                    else:
                        final_meds1=pd.concat([final_meds1,sub_meds])
                    t=t+1
                if final_meds.empty:
                    final_meds=final_meds1
                else:
                    final_meds=pd.concat([final_meds,final_meds1])

            ###PROC
            if(self.feat_proc):
                p1=self.proc[self.proc['stay_id']==hid]
                p=0
                for i in tqdm(range(0,len(p1),bucket)):
                    #sub_proc=self.proc[(self.proc['start_time']>=i) & (self.proc['start_time']<i+bucket)].groupby(['stay_id','itemid']).agg({'subject_id':'max'})
                    sub_proc=self.proc[(self.proc['start_time']>=i) & (self.proc['start_time']<i+bucket)].groupby(['stay_id','itemid']).agg({'start_time':'max','subject_id':'max','mdr_los':'min'})
                    sub_proc=sub_proc.reset_index()
                    sub_proc['mdr_label']=np.where((sub_proc['mdr_los']>i)&(sub_proc['mdr_los']<=i+bucket),1,0)
                    sub_proc['start_time']=p
                    sub_proc['seq']=int((i+bucket)/bucket -1)
                    if final_proc1.empty:
                        final_proc1=sub_proc
                    else:    
                        final_proc1=pd.concat([final_proc1,sub_proc])
                    p=p+1
                if final_proc.empty:
                    final_proc=final_proc
                else:
                    final_proc=pd.concat([final_proc,final_proc1])

            ###ANTI
            if(self.feat_anti):
                a1=self.anti[self.anti['stay_id']==hid]
                a=0
                for i in tqdm(range(0,len(a1),bucket)):
                    #sub_anti=self.anti[(self.anti['start_time']>=i) & (self.anti['start_time']<i+bucket)].groupby(['stay_id','antibiotic']).agg({'subject_id':'max'})
                    sub_anti=self.anti[(self.anti['start_time']>=i) & (self.anti['start_time']<i+bucket)].groupby(['stay_id','antibiotic']).agg({'start_time':'max','subject_id':'max','mdr_los':'min'})
                    sub_anti=sub_anti.reset_index()
                    sub_anti['mdr_label']=np.where((sub_anti['mdr_los']>i)&(sub_anti['mdr_los']<=i+bucket),1,0)
                    sub_anti['start_time']=a
                    sub_anti['seq']=int((i+bucket)/bucket -1)
                    if final_anti1.empty:
                        final_anti1=sub_anti
                    else:
                        final_anti1=pd.concat([final_anti1,sub_anti])
                    a=a+1
                if final_anti.empty:
                    final_anti=final_anti1
                else:
                    final_anti=pd.concat([final_anti,final_anti1])

            ###VENT
            if(self.feat_vent):
                v1=self.vent[self.vent['stay_id']==hid]
                v=0
                for i in tqdm(range(0,len(v1),bucket)):
                    #sub_vent=self.vent[(self.vent['start_time']>=i) & (self.vent['start_time']<i+bucket)].groupby(['stay_id','ventilation_status']).agg({'subject_id':'max'})
                    sub_vent=self.vent[(self.vent['start_time']>=i) & (self.vent['start_time']<i+bucket)].groupby(['stay_id','ventilation_status']).agg({'start_time':'max','subject_id':'max','mdr_los':'min'})
                    sub_vent=sub_vent.reset_index()
                    sub_vent['mdr_label']=np.where((sub_vent['mdr_los']>i)&(sub_vent['mdr_los']<=i+bucket),1,0)
                    sub_vent['start_time']=v
                    sub_vent['seq']=int((i+bucket)/bucket -1)
                    if final_vent1.empty:
                        final_vent1=sub_vent
                    else:
                        final_vent1=pd.concat([final_vent1,sub_vent])
                    v=v+1
                if final_vent.empty:
                    final_vent=final_vent1
                else:
                    final_vent=pd.concat([final_vent,final_vent1])

            ###OUT
            if(self.feat_out):
                o1=self.out[self.out['stay_id']==hid]
                o=0
                for i in tqdm(range(0,len(v1),bucket)):        
                    #sub_out=self.out[(self.out['start_time']>=i) & (self.out['start_time']<i+bucket)].groupby(['stay_id','itemid']).agg({'subject_id':'max'})
                    sub_out=self.out[(self.out['start_time']>=i) & (self.out['start_time']<i+bucket)].groupby(['stay_id','itemid']).agg({'start_time':'max','subject_id':'max','mdr_los':'min'})
                    sub_out=sub_out.reset_index()
                    sub_out['start_time']=o
                    sub_out['seq']=int((i+bucket)/bucket -1)
                    if final_out1.empty:
                        final_out1=sub_out
                    else:    
                        final_out1=pd.concat([final_out1,sub_out])
                    o=o+1
                if final_out.empty:
                    final_out=final_out1
                else:
                    final_out=pd.concat([final_out,final_out1])

            ###CHART
            if(self.feat_chart):
                c1=self.chart[self.chart['stay_id']==hid]
                c=0        
                for i in tqdm(range(0,len(c1),bucket)):         
                    #sub_chart=self.chart[(self.chart['start_time']>=i) & (self.chart['start_time']<i+bucket)].groupby(['stay_id','itemid']).agg({'valuenum':np.nanmean})
                    sub_chart=self.chart[(self.chart['start_time']>=i) & (self.chart['start_time']<i+bucket)].groupby(['stay_id','itemid']).agg({'start_time':'max','mdr_los':'min','valuenum':np.nanmean})
                    sub_chart=sub_chart.reset_index()
                    sub_chart['start_time']=c
                    sub_chart['seq']=int((i+bucket)/bucket -1)
                    if final_chart1.empty:
                        final_chart1=sub_chart
                    else:    
                        final_chart1=pd.concat([final_chart1,sub_chart])
                    c=c+1
                if final_chart.empty:
                    final_chart=final_chart1
                else:
                    final_chart=pd.concat([final_chart,final_chart1])
...
#             self.create_chartDict(final_chart,los)
#         else:
        self.create_Dict_all(final_data,final_meds,final_proc,final_out,final_anti,final_vent,final_chart,los)

    def create_chartDict(self,chart,los):
        dataDic={}
        for hid in self.hids:
            grp=self.data[self.data['stay_id']==hid]
            dataDic[hid]={'Chart':{},'label':int(grp['label_mdr48'])}
        for hid in tqdm(self.hids):
            ###CHART
            if(self.feat_chart):
...
#         print("# Unique insurance",self.data.insurance.nunique())
    ...
    def create_Dict_all(self,data,meds,proc,out,anti,vent,chart,los):
        dataDic={}
        #print(los)
        labels_csv=pd.DataFrame(columns=['stay_id','icu_mdr'])
        labels_csv['stay_id']=pd.Series(self.hids)
...
#         print("# Unique insurance",self.data.insurance.nunique())
        ...
        for hid in tqdm(self.hids):
            grp=self.data[self.data['stay_id']==hid]
            demo_csv=grp[['Age','gender','ethnicity','insurance']]
            if not os.path.exists("./data/csv_all"+str(hid)):
                os.makedirs("./data/csv_all/"+str(hid))
            demo_csv.to_csv('./data/csv_all/'+str(hid)+'/demo.csv',index=False)
            
            dyn_csv=pd.DataFrame()
            mdr_csv=pd.DataFrame()
            lens=int(grp['los']/self.bucket)
            ###DATA
            df3=data[data['stay_id']==hid]
            if df3.shape[0]==0:
                df3=pd.DataFrame(np.zeros([lens,1]))
                df3=df3.fillna(0)
                df3.columns=['mdr_label']
                df3['mdr_label']=df3['mdr_label'].astype(int)
            else:
                df3=df3.pivot_table(index='seq',columns='stay_id',values='icu_mdr')
                df3.columns=['mdr_label']
                add_indices = pd.Index(range(lens)).difference(df3.index)
                add_df = pd.DataFrame(index=add_indices, columns=df3.columns).fillna(np.nan)
                df3=pd.concat([df3, add_df])
                df3=df3.sort_index()
                df3=df3.ffill()
            ...
            if(self.feat_med):
                ...
                df2=meds[meds['stay_id']==hid]
                if df2.shape[0]==0:
                    amount=pd.DataFrame(np.zeros([lens,len(feat)]),columns=feat)
                    amount=amount.fillna(0)
                    amount.columns=pd.MultiIndex.from_product([["MEDS"], amount.columns])
                else:
                    rate=df2.pivot_table(index='start_time',columns='itemid',values='rate')
                    ...
                    df2=df2.pivot_table(index='start_time',columns='itemid',values='stop_time')
                    #print(df2.shape)
                    add_indices = pd.Index(range(lens)).difference(df2.index)
                    add_df = pd.DataFrame(index=add_indices, columns=df2.columns).fillna(np.nan)
                    df2=pd.concat([df2, add_df])
                    df2=df2.sort_index()
                    df2=df2.ffill()
            ...
            if(self.feat_proc):
                ...
                df2=proc[proc['stay_id']==hid]
                if df2.shape[0]==0:
                    df2=pd.DataFrame(np.zeros([lens,len(feat)]),columns=feat)
                    df2=df2.fillna(0)
                    df2.columns=pd.MultiIndex.from_product([["PROC"], df2.columns])
                else:
                    df2['val']=1
            ...
            if(self.feat_chart):
                ...
                df2=chart[chart['stay_id']==hid]
                if df2.shape[0]==0:
                    val=pd.DataFrame(np.zeros([lens,len(feat)]),columns=feat)
                    val=val.fillna(0)
                    val.columns=pd.MultiIndex.from_product([["CHART"], val.columns])
                else:
                    val=df2.pivot_table(index='start_time',columns='itemid',values='valuenum')
            ...
            #Save temporal data to csv
            dyn_csv.to_csv('./data/csv_all/'+str(hid)+'/dynamic.csv',index=False)
            mdr_csv.to_csv('./data/csv_all/'+str(hid)+'/mdr.csv',index=False)
            ...
            if(self.feat_cond):
                ...
                if(grp.shape[0]==0):
                    ...
                    grp=grp[feat]
                    grp.columns=pd.MultiIndex.from_product([["COND"], grp.columns])
            grp.to_csv('./data/csv_all/'+str(hid)+'/static.csv',index=False)   
            labels_csv.to_csv('./data/csv_all/labels.csv',index=False)    
            
                
        ######SAVE DICTIONARIES##############
        metaDic={'Cond':{},'Proc':{},'Med':{},'Out':{},'Chart':{},'Anti':{},'Vent':{},'LOS':{}}
        metaDic['LOS']=los
        with open("./data/dict_all/dataDic", 'wb') as fp:
            pickle.dump(dataDic, fp)

        with open("./data/dict_all/hadmDic", 'wb') as fp:
            pickle.dump(self.hids, fp)
        
        with open("./data/dict_all/ethVocab", 'wb') as fp:
            pickle.dump(list(self.data['ethnicity'].unique()), fp)
            self.eth_vocab = self.data['ethnicity'].nunique()
            
        with open("./data/dict_all/ageVocab", 'wb') as fp:
            pickle.dump(list(self.data['Age'].unique()), fp)
            self.age_vocab = self.data['Age'].nunique()
            
        with open("./data/dict_all/insVocab", 'wb') as fp:
            pickle.dump(list(self.data['insurance'].unique()), fp)
            self.ins_vocab = self.data['insurance'].nunique()
            
        if(self.feat_med):
            with open("./data/dict_all/medVocab", 'wb') as fp:
                pickle.dump(list(meds['itemid'].unique()), fp)
            self.med_vocab = meds['itemid'].nunique()
            metaDic['Med']=self.med_per_adm
        ...
        if(self.feat_chart):
            with open("./data/dict_all/chartVocab", 'wb') as fp:
                pickle.dump(list(chart['itemid'].unique()), fp)
            self.chart_vocab = chart['itemid'].nunique()
            metaDic['Chart']=self.chart_per_adm
        
        if(self.feat_cond):
            with open("./data/dict_all/condVocab", 'wb') as fp:
                pickle.dump(list(self.cond['new_icd_code'].unique()), fp)
            self.cond_vocab = self.cond['new_icd_code'].nunique()
            metaDic['Cond']=self.cond_per_adm
        
        if(self.feat_proc):    
            with open("./data/dict_all/procVocab", 'wb') as fp:
                pickle.dump(list(proc['itemid'].unique()), fp)
            self.proc_vocab = proc['itemid'].nunique()
            metaDic['Proc']=self.proc_per_adm

        if(self.feat_anti):
            with open("./data/dict_all/antiVocab", 'wb') as fp:
                pickle.dump(list(anti['antibiotic'].unique()), fp)
            self.anti_vocab = anti['antibiotic'].nunique()
            metaDic['Anti']=self.anti_per_adm
        
        if(self.feat_vent):
            with open("./data/dict_all/ventVocab", 'wb') as fp:
                pickle.dump(list(vent['ventilation_status'].unique()), fp)
            self.vent_vocab = vent['ventilation_status'].nunique()
            metaDic['Vent']=self.vent_per_adm
            
        with open("./data/dict_all/metaDic", 'wb') as fp:
            pickle.dump(metaDic, fp) 
