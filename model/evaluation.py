...
class Loss(nn.Module):
    ...
    def forward(self, prob,labels,logits, contrastive_loss,train=True, standalone=False,contrastive_weight=args.contrastive_weight):
        classify_loss='NA' 
        auc,apr='NA'
        base='NA'
        accur='NA'
        ...
        print("BCE Loss: {:.5f}".format(classify_loss))
        print("Total Loss: {:.5f}".format(total_loss)) 
        print("AU-ROC: {:.3f}".format(auc))
        print("AU-PRC: {:.3f}".format(apr))
        print("AU-PRC Baseline: {:.2f}".format(base))
        print("Accuracy: {:.3f}".format(accur))
    ...
    def calb_curve(self,bins,bin_accs,ECE, MCE):
        ...
        plt.savefig(f'./data/output{args.data_name}/'+"callibration_plot.png")
        #plt.show()
